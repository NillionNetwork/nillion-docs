...
Build a React app that reads all records from a [Nillion Private Storage](/blind-computer/build/storage/overview) standard collection entirely in the browser with no server required!

:::warning
While this browser-only approach is possible with the configuration shown, we recommend using the [Next.js recipe](/blind-computer/build/storage/platform-nextjs) or a separate browser/server integration for better security and maintainability.
:::
