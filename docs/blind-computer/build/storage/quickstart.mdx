...
- How sessions persist via **invocation tokens**
...
- `initializeSession()`:
  ...
  - Returns `{ nillionClient, nildbTokens }`.
...
- Rehydrates from localStorage (`nildbTokens`)
...
6. Sessions persist via per-node invocation tokens in localStorage
