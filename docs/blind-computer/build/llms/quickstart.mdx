...
With nilAI, it is possible to run AI models within a trusted execution environment (TEE). This quickstart uses the latest `secretllm_nextjs` example to build a private chat app with API key authentication.
...
Now we need to set `NILLION_API_KEY` using a DID private key created via the nilAI Developer Dashboard.

<CTABanner
  question='Get your nilAI API keys here'
  instruction='Visit the nilAI Developer Dashboard and create a public DID / private API key'
