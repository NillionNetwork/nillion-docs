...
Nillion Blacklight is a decentralised verification network designed to continuously verify workloads running inside Trusted Execution Environments (TEEs). The system is composed of three core components that work together to provide ongoing, credibly neutral verification:
    •   TEE Operators, who run workloads inside TEEs and submit proofs of liveness
    •   Nillion’s L2, which coordinates verification, consensus, and rewards
    •   Blacklight Nodes, which independently challenge and verify TEE attestations

Together, these components enable continuous, decentralised verification of TEE-backed workloads. Providing a credibly neutral trust layer for TEEs.
