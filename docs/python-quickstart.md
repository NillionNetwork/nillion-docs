...
Once you have finished, explore more examples and tutorials in the QuickStart repo to continue your Nillion developer journey!
...
1. Fork the repo on GitHub - it will be forked into a repo called `your-username/nillion-python-starter`
...
The Nillion Network uses [Nada](https://docs.nillion.com/nada-lang), our MPC language, to define MPC programs. The first implementation of Nada is a Python DSL (Domain Specific Language), called Nada. In this section we will write a simple program that adds two numbers together. The code for the finished program is below.
...
```bash
cd quickstart/nada_programs/src
touch secret_addition.py
```
2. Write the program above into this file

### Understanding the program you have just written
1. First we must import nada_dsl and create a function nada_main() - this function Awill contain our programs code.
```python
from nada_dsl import *
...
Make sure you are in the `quickstart/nada-programs` directory.
...
The directory structure inside the `nada_programs` directory has been created by running `nada init <your-project-name>`. You should do this everytime you want to set up a new project.
...
Finally, we test the program. If you run the above command without altering the default values (3s) in the test file (tests/secret_addition_test.yaml), the test will fail.
...
We will write the following code in the `quickstart/client_code/secret_addition.py` file. You can view the completed code here, feel free to refer back to it whenever you need.
